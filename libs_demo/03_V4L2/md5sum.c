 ...
 * @return false
 */
bool get_md5sum_from_data(uint8_t *data, int dataLen, char *md5sum, int md5sumSize)
{
    if (NULL == data || dataLen <= 0 || NULL == md5sum || md5sumSize < MD5SUM_CHAR_NUM)
    {
        return false;
    }
	...
	md5_init (&ctx);
	md5_write (&ctx, data, dataLen);
	md5_final (&ctx);

    for (int i = 0; i < MD5SUM_CHAR_NUM/2; i++)
    {
        snprintf(md5sum + i*2, 3,"%02x", ctx.buf[i]);
    }
